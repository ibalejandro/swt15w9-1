...
public class GoodsOfferControllerIntegrationTests extends AbstractWebIntegrationTests {
	...
	public void testSaveGood() throws Exception {
		mvc.perform(post("/offeredGood").param("name", good1.getName()).param("description", good1.getDescription())
				.param("tagId", String.valueOf(good1.getTag().getId()))).andExpect(status().isOk())
				.andExpect(model().attribute("result", is(not(emptyIterable()))))
				.andExpect(model().attribute("result", Matchers.hasProperty("name", Matchers.equalTo(good1.getName()))))
				.andExpect(model().attribute("result",
						Matchers.hasProperty("description", Matchers.equalTo(good1.getDescription()))))
				/*
				 * .andExpect (model().attribute("result",
				 ...
				 */
				.andExpect(
						model().attribute("result", Matchers.hasProperty("user", Matchers.equalTo(good1.getUser()))));

		mvc.perform(post("/offeredGood").param("name", good2.getName()).param("description", good2.getDescription())
				.param("tagId", String.valueOf(good2.getTag().getId()))).andExpect(status().isOk())
				.andExpect(model().attribute("result", is(not(emptyIterable()))))
				.andExpect(model().attribute("result", Matchers.hasProperty("name", Matchers.equalTo(good2.getName()))))
				.andExpect(model().attribute("result",
						Matchers.hasProperty("description", Matchers.equalTo(good2.getDescription()))))
				/*
				 * .andExpect (model().attribute("result",
				 * Matchers.hasProperty("tag", Matchers.equalTo
				 * (good2.getTag()))))
				 */
				.andExpect(
						model().attribute("result", Matchers.hasProperty("user", Matchers.equalTo(good2.getUser()))));
	}
	...
	public void testListAllGoods() {
		...
		Iterable<Object> object = (Iterable<Object>) model.asMap().get("resultGoods");

...
public class GoodsSearchControllerIntegrationTests extends AbstractWebIntegrationTests {
	...
	public void testSearchGoodByTag() throws Exception {
		mvc.perform(post("/searchResultsByTag").param("tagId", String.valueOf(tag1.getId())).param("distance", "-1"))
				.andExpect(status().isOk()).andExpect(model().attribute("resultGoods", is(not(emptyIterable()))))
				.andExpect(model().attribute("resultParameter", Matchers.equalTo(tag1.getName())))
				.andExpect(model().attribute("resultGoods", is(iterableWithSize(iterableSizeForTagSportingGoods))));

		mvc.perform(post("/searchResultsByTag").param("tagId", String.valueOf(tag2.getId())).param("distance", "-1"))
				.andExpect(status().isOk()).andExpect(model().attribute("resultGoods", is(not(emptyIterable()))))
				.andExpect(model().attribute("resultParameter", Matchers.equalTo(tag2.getName()))).andExpect(model()
						.attribute("resultGoods", is(iterableWithSize(iterableSizeForTagClothesShoesAndAccesories))));

		mvc.perform(post("/searchResultsByTag").param("tagId", String.valueOf(tag3.getId())).param("distance", "-1"))
				.andExpect(status().isOk()).andExpect(model().attribute("resultGoods", is(not(emptyIterable()))))
				.andExpect(model().attribute("resultParameter", Matchers.equalTo(tag3.getName())))
				.andExpect(model().attribute("resultGoods", is(iterableWithSize(iterableSizeForTagDollsAndBears))));

		mvc.perform(post("/searchResultsByTag").param("tagId", String.valueOf(nonMatchingTag.getId())).param("distance",
				"-1")).andExpect(status().isOk()).andExpect(model().attribute("resultGoods", is(emptyIterable())))
				.andExpect(model().attribute("resultParameter", Matchers.equalTo(nonMatchingTag.getName())));

		// A tag's id equals -1 means that all goods have to be returned.
		mvc.perform(post("/searchResultsByTag").param("tagId", "-1").param("distance", "-1")).andExpect(status().isOk())
				.andExpect(model().attribute("resultGoods", is(not(emptyIterable()))))
				.andExpect(model().attribute("resultParameter", Matchers.equalTo("All"))).andExpect(model()
						.attribute("resultGoods", is(iterableWithSize(iterableSizeForAllGoods + goodsInTestDaten))));
	}

...
public class GeoCodingTest extends AbstractWebIntegrationTests {
	...
	public void modifyAddressHelper() throws Exception {
		...
		mvc.perform(
				get("/modifyAddress").with(user("testUser1").roles("NORMAL"))).//
				andExpect(status().isOk()).//
				andExpect(view().name("modifyAddress")).//
				andExpect(model().attributeExists("userAccount"));

		mvc.perform(
				post("/modifyAddress_submit").param("wohnen", "helper").with(
						user("testUser1").roles("NORMAL"))).//
				andExpect(view().name("redirect:/data"));

		mvc.perform(
				post("/modifyAddress_submit").param("wohnen", "helper")
						.param("street", "Prager Str.").param("housenr", "10")
		...
		mvc.perform(
				post("/modifyAddress_submit").param("wohnen", "helper")
						...
						.param("housenr", "46").param("postcode_H", "01187")
						.with(user("testUser1").roles("NORMAL"))).//
				andExpect(view().name("redirect:/data"));
		...
		mvc.perform(
				post("/modifyAddress_submit").param("wohnen", "helper")
						.param("street", "Altplauen").param("housenr", "19")
						.with(user("testUser1").roles("NORMAL"))).//
				andExpect(view().name("redirect:/data"));
	...
	public void modifyAddressRefugee() throws Exception {
		...
		mvc.perform(
				post("/modifyAddress_submit").param("wohnen", "refugee")
		...
		user3 = userRepository.findByUserAccount(userAccountManager
				.findByUsername("testUser1").get());
	...
	public void modifyAddressAdmin() throws Exception {
		...
		mvc.perform(
				post("/modify_submit/user/{user}", user)
						.param("wohnen", "helper")
						.param("street", "Prager Str.").param("housenr", "10")
						.param("postcode_H", "01069")
						.param("city_H", "Dresden")
	...
	/*
	 * @Test public void registerAddressHelper() throws Exception{ User user1=
	 * userRepository
	 * .findByUserAccount(userAccountManager.findByUsername("testUser1").get());
	 * String user="testUser1"; double delta=0.00001;
	 * 
	 * mvc.perform(post("/create_new_user") .param("mailIN", "test@test.test")
	 * .param("usernameIN", "Test") .param("passwordIN", "Test08.12")
	 * .param("repasswordIN", "Test08.12") .param("nameIN", "Abc")
	 * .param("firstnameIN", "Def") .param("wohnen", "helper") .param("street",
	 * "Prager Str.") .param("housenr", "10") .param("postcode_H", "01069")
	 * .param("city_H", "Dresden") .param("nativelanguage", "de")
	 * .param("otherlanguages", "en") .param("origin", "Deutschland")
	 * .param("g-recaptcha-response", "en")
	 * .with(user("testUser1").roles("NORMAL"))).//
	 * andExpect(view().name("redirect:/userDetails"));
	 * 
	 * user1=
	 * userRepository.findByUserAccount(userAccountManager.findByUsername(
	 * "Test").get()); assertEquals(51.045846, user1.getLatitude(), delta);
	 * assertEquals(13.736928, user1.getLongitude(),delta); }
	 */
